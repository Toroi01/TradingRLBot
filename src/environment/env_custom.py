...
class CustomTradingEnv(gym.Env):
    ...
    def __init__(self, df, main_tickers, all_tickers, max_assets_amount_per_trade, features,
        ...
        if not self.discrete_actionspace:
            ...
            self.action_space = spaces.Discrete(2 * len(self.main_tickers))
            self.observation_space = spaces.Box(low=-np.inf, high=np.inf,
                                                shape=(self.state.get_size(self.main_tickers, self.all_tickers),))

        self._hour_counter = min(self.df.index)
        self._episode = 0
        self._historic_allocation_amount = []
        self._historic_allocation_values = []
    ...
    def step(self, actions):
        ...
        """
        # Update the hourly data and calculate the current portfolio value
        ...
        if self.discrete_actionspace == True:
            actions = self.transform_discrete_actions(actions)
    ...
    def log_allocation_amount(self, hourly_data):
        amounts = self.portfolio.to_dict()
        amounts['date'] = hourly_data.date.iloc[0]
        self._historic_allocation_amount.append(amounts)

    def log_allocation_values(self, hourly_data):
        assets_values = {k: v for k, v in self.portfolio.get_assets_value(hourly_data).items()}
        assets_values['date'] = hourly_data.date.iloc[0]
        self._historic_allocation_values.append(assets_values)
    ...
    def save_asset_memory(self):
        return pd.DataFrame(self._historic_allocation_amount)

    def save_action_memory(self):
        return self.portfolio.get_historic_transactions()

    def save_asset_values_memory(self):
        return pd.DataFrame(self._historic_allocation_values)
    ...
    def transform_discrete_actions(self, action):
        """
        Input: 0
        Output: [0, 0]
        ...
        if action == 0:
            # Don't do anything
            return np.zeros(len(self.main_tickers))

        binary_action_space = np.zeros(len(self.main_tickers * 2))
        binary_action_space[action - 1] = 1

        mapped_actions = []
        for i in range(0, len(self.main_tickers * 2), 2):
            ticker_actions = binary_action_space[i:i + 2]
            # Transforming to -1, 1
            value = ticker_actions[0] * -1 + ticker_actions[1] * 1
            mapped_actions.append(value)
